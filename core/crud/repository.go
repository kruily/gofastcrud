...
func NewRepository[T ICrudEntity](db *database.Database, entity T) *Repository[T] {
	...
	case DB_TYPE_GORM:
		repo.crudRepo = newGormRepository(db.DB(), entity)
	case DB_TYPE_MONGODB:
		repo.crudRepo = newMongoRepository(db.MDB(), entity)
	}
